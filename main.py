...
dftest.loc['One RF', 'RMSE'] = np.sqrt(mean_squared_error(rfonerandomforest.predict(X.loc[((X.index>=pd.to_datetime(d2)) & (X.index < pd.to_datetime(d3))),colused]), 
...
dfcv.to_csv('dfcv_ran.csv')
dftest.to_csv('dftest_ran.csv')
